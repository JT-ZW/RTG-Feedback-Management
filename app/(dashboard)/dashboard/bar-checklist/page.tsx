...
import type { BarChecklistResponses } from '@/lib/bar-checklist-items'
...
export default async function BarChecklistAdminPage() {
  ...
  let query = admin
    ...
    .select('id, property_id, submission_date, outlet, submitter_name, position, responses, yes_count, total_items, compliance_score, status, created_at')
  ...
  for (const item of BAR_CHECKLIST_ITEMS) noCountByItem[item.id] = 0

  for (const s of subs) {
    let parsed: BarChecklistResponses = {}
  ...
  const topFailItems = [...BAR_CHECKLIST_ITEMS]
    .sort((a, b) => (noCountByItem[b.id] ?? 0) - (noCountByItem[a.id] ?? 0))
    .slice(0, 5)
    .filter(item => (noCountByItem[item.id] ?? 0) > 0)

  const publicFormUrl = `${process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3000'}/forms/bar-checklist`
    ...
    <div className="max-w-6xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-stone-900">Bar Checklist</h1>
      ...
      {subs.length === 0 ? (
        <div className="bg-white rounded-xl border border-stone-200 py-16 text-center">
          <Wine className="w-8 h-8 text-stone-300 mx-auto mb-3" />
          <p className="text-stone-500 text-sm font-medium">No submissions yet</p>
          <p className="text-stone-400 text-xs mt-1">Share the bar checklist form link above to start collecting reports.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-stone-100 bg-stone-50">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-stone-500 uppercase tracking-wide">Outlet</th>
                ...
                {subs.map(sub => {
                  ...
                  return (
                    <tr key={sub.id} className="hover:bg-stone-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-stone-600">{sub.outlet}</td>
