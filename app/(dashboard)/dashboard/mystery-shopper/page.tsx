...
import { ShieldCheck, ExternalLink, TrendingUp, TrendingDown, Award, AlertTriangle, Building2, CalendarCheck } from 'lucide-react'
...
function tileGradeClass(pct: number) {
  if (pct >= 90) return { bg: 'bg-emerald-50 border-emerald-200', value: 'text-emerald-700', label: 'text-emerald-600' }
  if (pct >= 75) return { bg: 'bg-lime-50 border-lime-200', value: 'text-lime-700', label: 'text-lime-600' }
  if (pct >= 60) return { bg: 'bg-amber-50 border-amber-200', value: 'text-amber-700', label: 'text-amber-600' }
  return { bg: 'bg-red-50 border-red-200', value: 'text-red-700', label: 'text-red-600' }
}

export default async function MysteryShopperAdminPage() {
  ...
  let query = admin
    ...
    .select('id, property_id, visit_date, shopper_name, total_score, max_score, percentage, status, created_at')
  ...
  // Fetch all properties (not just those in submissions) to show coverage
  ...
  const bestProperty = propertyAvgs.length > 0
    ? propertyAvgs.reduce((a, b) => (a.avg >= b.avg ? a : b))
    : null
  const lowestProperty = propertyAvgs.length > 0
    ? propertyAvgs.reduce((a, b) => (a.avg <= b.avg ? a : b))
    : null

  const propertiesAudited = propertyAvgs.length
  ...
  const publicFormUrl = `${process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3000'}/forms/mystery-shopper`
    ...
    <div className="max-w-6xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-stone-900">Mystery Shopper Checklist</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {/* Group Average */}
        <div className={`rounded-xl border p-5 ${groupAvg != null ? tileGradeClass(groupAvg).bg : 'bg-stone-50 border-stone-200'}`}>
          ...
          {groupAvg != null ? (
            <>
              <p className={`text-3xl font-bold ${tileGradeClass(groupAvg).value}`}>
                {groupAvg.toFixed(1)}<span className="text-lg font-normal">%</span>
              </p>
              <p className={`text-xs font-medium mt-1 ${tileGradeClass(groupAvg).label}`}>
                {scoreLabel(groupAvg)}
              </p>
            </>
          ) : (
        ...
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-stone-500 uppercase tracking-wide">Top Performer</span>
          ...
          {bestProperty ? (
            <>
              ...
              <p className="text-xs text-emerald-600 mt-0.5">{bestProperty.count} audit{bestProperty.count !== 1 ? 's' : ''}</p>
            </>
          ) : (
            <p className="text-2xl font-bold text-stone-300">—</p>
          )}
        ...
        {/* Lowest Property (only meaningful if >1 property) */}
        <div className={`rounded-xl border p-5 ${lowestProperty && lowestProperty.id !== bestProperty?.id ? 'border-red-200 bg-red-50' : 'border-stone-200 bg-stone-50'}`}>
          ...
          {lowestProperty && lowestProperty.id !== bestProperty?.id ? (
            <>
              ...
              <p className="text-xs text-red-600 mt-0.5">{lowestProperty.count} audit{lowestProperty.count !== 1 ? 's' : ''}</p>
            </>
          ) : lowestProperty ? (
            <>
              <p className="text-sm font-bold text-stone-600 leading-tight">{lowestProperty.name}</p>
              <p className="text-xs text-stone-400 mt-1">Only property audited</p>
            </>
          ) : (
            <p className="text-2xl font-bold text-stone-300">—</p>
          )}
        ...
        <div className="rounded-xl border border-stone-200 bg-white p-5">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-stone-500 uppercase tracking-wide">Properties Audited</span>
            <Building2 className="w-4 h-4 text-stone-400" />
          </div>
          <p className="text-3xl font-bold text-stone-800">
            {propertiesAudited}
            <span className="text-lg font-normal text-stone-400">/{totalProperties}</span>
          </p>
          {totalProperties > 0 && (
            <div className="mt-2 h-1.5 bg-stone-100 rounded-full overflow-hidden">
              <div
                className="h-full bg-violet-500 rounded-full"
                style={{ width: `${(propertiesAudited / totalProperties) * 100}%` }}
              />
            </div>
          )}
          <p className="text-xs text-stone-400 mt-1.5">
            {totalProperties - propertiesAudited > 0
              ? `${totalProperties - propertiesAudited} not yet audited`
        ...
        {/* Below Threshold */}
        <div className={`rounded-xl border p-5 ${belowThresholdCount > 0 ? 'border-orange-200 bg-orange-50' : 'border-stone-200 bg-white'}`}>
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-stone-500 uppercase tracking-wide">Below 60%</span>
            <AlertTriangle className={`w-4 h-4 ${belowThresholdCount > 0 ? 'text-orange-400' : 'text-stone-300'}`} />
          </div>
          <p className={`text-3xl font-bold ${belowThresholdCount > 0 ? 'text-orange-700' : 'text-stone-800'}`}>
            {belowThresholdCount}
          </p>
          <p className={`text-xs mt-1 ${belowThresholdCount > 0 ? 'text-orange-600' : 'text-stone-400'}`}>
            {belowThresholdCount > 0
              ? `audit${belowThresholdCount !== 1 ? 's' : ''} requiring follow-up`
              : 'No audits below threshold'}
          </p>
        </div>
      </div>

      {/* ── Per-property averages bar ─────────────────────────────────────────── */}
        ...
        <div className="bg-white rounded-xl border border-stone-200 p-5">
          ...
          <div className="space-y-3">
            {[...propertyAvgs].sort((a, b) => b.avg - a.avg).map(prop => {
              const c = tileGradeClass(prop.avg)
              return (
                <div key={prop.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 bg-stone-100 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${
                        prop.avg >= 90 ? 'bg-emerald-500'
                        : prop.avg >= 75 ? 'bg-lime-500'
                        : prop.avg >= 60 ? 'bg-amber-500'
                        : 'bg-red-500'
                      }`}
                  ...
                  <span className={`text-xs font-semibold w-12 text-right ${
                    prop.avg >= 90 ? 'text-emerald-700'
                    : prop.avg >= 75 ? 'text-lime-700'
                    : prop.avg >= 60 ? 'text-amber-700'
                    : 'text-red-700'
                  }`}>
                    {prop.avg.toFixed(1)}%
                  </span>
                  <span className="text-xs text-stone-400 w-14 text-right">{prop.count} audit{prop.count !== 1 ? 's' : ''}</span>
                </div>
              )
            })}
          </div>
      ...
      {!submissions || submissions.length === 0 ? (
        <div className="bg-white rounded-xl border border-stone-200 py-16 text-center">
          ...
          <p className="text-stone-400 text-xs mt-1">
            Share the public form link above to start collecting mystery shopper data.
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-stone-100 bg-stone-50">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-stone-500 uppercase tracking-wide">Shopper</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-stone-500 uppercase tracking-wide">Visit Date</th>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-stone-500 uppercase tracking-wide">Submitted</th>
                  ...
                  return (
                    <tr key={sub.id} className="hover:bg-stone-50 transition-colors">
                      <td className="px-4 py-3 font-medium text-stone-800">
                        {propMap[sub.property_id] ?? '—'}
                      </td>
                      <td className="px-4 py-3 text-stone-600">{sub.shopper_name}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${scoreColor(pct)}`}>
                          {pct.toFixed(1)}% — {scoreLabel(pct)}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link
                          href={`/dashboard/mystery-shopper/${sub.id}`}
