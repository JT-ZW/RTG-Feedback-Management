...
export function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // The server action sets rtg_last_active as httpOnly and redirects on success.
    // On error it returns { error } — the redirect throws so we only reach here if
    // the call returns normally (i.e. an auth error occurred).
    const result = await login(formData)
    if (result?.error) {
      setError('Invalid email or password. Please try again.')
      setLoading(false)
    }
    // On success the server action redirects, so no client-side navigation needed.
  }
